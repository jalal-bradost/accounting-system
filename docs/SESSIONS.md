...
- **Accounting integration:** new `JournalEntryPostingPort` consumed from inventory; `JournalEntryPostingAdapter` in `accounting-container` routes inventory entries to the seeded `INV` journal. Receipts post `Dr Stock Valuation / Cr Stock Input` (supplier) or `Dr Stock Valuation / Cr Stock Output` (customer return). Sales deliveries post `Dr Stock Output / Cr Stock Valuation`. COGS clearing (`Dr COGS / Cr Stock Output`) is reconciled to `min(qty_delivered, qty_invoiced)` after delivery sync or invoice post (supports invoice-before-delivery). Inventory adjustments post directly to COGS. Account ids resolved per-product or per-category.
