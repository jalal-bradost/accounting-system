...
ALTER TABLE hr_employee ADD COLUMN user_id CHAR(36) NULL;

ALTER TABLE hr_employee ADD CONSTRAINT fk_hr_employee_user
    FOREIGN KEY(user_id) REFERENCES platform_app_user(id);

-- One linked user per company; NULL user_id rows are allowed (MySQL treats NULLs as distinct in unique indexes).
CREATE UNIQUE INDEX ux_hr_employee_company_user
    ON hr_employee(company_id, user_id);

-- Salary rule GL account mapping
ALTER TABLE pay_salary_rule ADD COLUMN account_id CHAR(36) NULL;

ALTER TABLE pay_salary_rule ADD CONSTRAINT fk_pay_salary_rule_account
    FOREIGN KEY(account_id) REFERENCES accounts(id);

-- Remove hardcoded IQD default on contracts (MySQL: MODIFY without DEFAULT)
ALTER TABLE pay_contract MODIFY currency_code VARCHAR(8) NOT NULL;
