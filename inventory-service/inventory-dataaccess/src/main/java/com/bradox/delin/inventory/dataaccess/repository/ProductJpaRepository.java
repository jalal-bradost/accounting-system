    ...
    @Query("""
        ...
        WHERE p.companyId = :companyId
          ...
          AND (
                ...
                OR EXISTS (
                    SELECT parent FROM ProductEntity parent
                    WHERE parent.id = p.parentProductId
                      AND parent.saleOk = TRUE
        ...
        ORDER BY p.name ASC
        """)
    Page<ProductEntity> searchSaleable(@Param("companyId") UUID companyId,
                                       @Param("query") String query,
                                       Pageable pageable);

    java.util.List<ProductEntity> findByParentProductId(UUID parentProductId);
