...
import com.bradox.delin.inventory.domain.core.entity.StockMove;
import com.bradox.delin.inventory.domain.core.entity.StockPicking;
import com.bradox.delin.inventory.domain.core.entity.StockQuant;
import com.bradox.delin.inventory.domain.core.entity.StockValuationLayer;
import com.bradox.delin.inventory.domain.core.exception.InventoryDomainException;
import com.bradox.delin.inventory.domain.core.service.ValuationContext;
import com.bradox.delin.inventory.domain.core.service.ValuationResult;
import com.bradox.delin.inventory.domain.core.service.ValuationStrategy;
import com.bradox.delin.inventory.domain.core.service.ValuationStrategyFactory;
...
import com.bradox.delin.inventory.domain.core.valueobject.StockQuantId;
import com.bradox.delin.inventory.domain.core.valueobject.ValuationLayerId;
import com.bradox.delin.inventory.domain.core.valueobject.ValuationMethod;
import com.bradox.delin.inventory.service.domain.dto.CreateStockPickingCommand;
import com.bradox.delin.inventory.service.domain.dto.InventoryAdjustmentCommand;
...
import com.bradox.delin.inventory.service.domain.ports.output.accounting.JournalEntryPostingPort;
import com.bradox.delin.inventory.service.domain.ports.output.repository.ProductCategoryRepository;
import com.bradox.delin.inventory.service.domain.ports.output.messaging.InventoryEventPublisher;
...
import com.bradox.delin.inventory.service.domain.ports.output.repository.StockQuantRepository;
...
import com.bradox.delin.inventory.service.domain.ports.output.repository.StockValuationLayerRepository;
...
class StockPickingApplicationServiceImpl implements StockPickingApplicationService {
    ...
    public StockPickingResponse validatePicking(UUID pickingId, ValidatePickingCommand command) {
        ...
        for (StockMove move : picking.getMoves()) {
            ...
            if (backorderQty.signum() > 0 && command != null && command.isCreateBackorder()) {
                StockMove bo = StockMove.builder()
                        .id(new com.bradox.delin.inventory.domain.core.valueobject.StockMoveId(UUID.randomUUID()))
                        .productId(move.getProductId())
                        .uomId(move.getUomId())
        ...
        if (savedPicking.getPurchaseOrderId() != null
            ...
            if (sync != null) {
                sync.afterIncomingPickingValidated(poId, savedPicking.getId().getId());
            }
        }
        if (savedPicking.getSalesOrderId() != null
            ...
            if (salesSync != null) {
                salesSync.afterOutgoingPickingValidated(soId, savedPicking.getId().getId());
            }
        }
    ...
    public StockPickingResponse returnPicking(UUID pickingId) {
        return returnPicking(pickingId, null);
    }

    @Override
    @Transactional
    public StockPickingResponse returnPicking(UUID pickingId, ReturnPickingCommand command) {
        ...
        List<StockMove> reversed = original.getMoves().stream()
                ...
                .map(m -> {
                    UUID moveId = m.getId() != null ? m.getId().getId() : null;
                    BigDecimal qty = qtyOverrides.containsKey(moveId)
                            ? qtyOverrides.get(moveId)
                            : m.getPickedQuantity();
                    if (qty == null || qty.signum() <= 0) {
                        return null;
                    }
                    if (m.getPickedQuantity() != null && qty.compareTo(m.getPickedQuantity()) > 0) {
                        throw new InventoryDomainException(
                                "error.inventory.returnQtyExceedsPicked",
                                null,
        ...
        StockPicking returnPicking = StockPicking.builder()
                ...
                .toRefund(command == null || command.isToRefund())
                .build();
    ...
    private void addJournalLines(List<JournalEntryPostingPort.JournalLine> lines,
        ...
        if (isReceiptForProduct(type, source, destination)) {
            ...
            LocationType from = source.getLocationType();
            if (from == LocationType.CUSTOMER) {
                // Customer return: reverse sales delivery interim (Dr Inventory, Cr Stock Output)
                require(stockOutput, "stock_output_account", product);
                lines.add(new JournalEntryPostingPort.JournalLine(stockValuation,
                        "Stock return " + sku, value, Money.ZERO));
                ...
                lines.add(new JournalEntryPostingPort.JournalLine(stockValuation,
                        "Stock IN " + sku, value, Money.ZERO));
                lines.add(new JournalEntryPostingPort.JournalLine(stockInput,
                        "Stock IN " + sku, Money.ZERO, value));
            }
        } else if (isDeliveryForProduct(type, source, destination)) {
            require(stockValuation, "stock_valuation_account", product);
            LocationType to = destination.getLocationType();
            if (to == LocationType.CUSTOMER) {
                // Sales delivery: interim on BS until invoice clears to COGS
                // Dr Stock Output, Cr Inventory
                require(stockOutput, "stock_output_account", product);
                lines.add(new JournalEntryPostingPort.JournalLine(stockOutput,
                        "Stock OUT " + sku, value, Money.ZERO));
                lines.add(new JournalEntryPostingPort.JournalLine(stockValuation,
                ...
                require(stockInput, "stock_input_account", product);
                lines.add(new JournalEntryPostingPort.JournalLine(stockInput,
                        "Stock return to vendor " + sku, value, Money.ZERO));
                lines.add(new JournalEntryPostingPort.JournalLine(stockValuation,
                        "Stock return to vendor " + sku, Money.ZERO, value));
            } else {
                // Other outgoing counterparties: expense immediately
                require(cogs, "cogs_account", product);
                lines.add(new JournalEntryPostingPort.JournalLine(cogs,
                        "Stock OUT " + sku, value, Money.ZERO));
                lines.add(new JournalEntryPostingPort.JournalLine(stockValuation,
    ...
    private StockPickingResponse toResponse(StockPicking picking) {
        StockPickingResponse response = mapper.pickingToResponse(picking);
        CompanyContext ctx = companyContextProvider.getIfAvailable();
        if (ctx != null) {
            response.setValidatedBy(ctx.resolveUserDisplay(response.getValidatedBy()));
        }
        return response;
    }
}
